...
// 16. Technical Rooms (232 rows)
export const generateTechnicalRoomsData = (): HSRRow[] => {
  const rooms = ['CER (Telecom Equipment Room)', 'TER (Technical Equipment Room)', 'SER (Signaling Equipment Room)', 'Battery Room'];
  const systems = ['Telecom Main Racks', 'Optical Distribution Frame', 'UPS Feeder Board', 'Cable Ladder & Trays'];
  const rows: HSRRow[] = [];
  let counter = 1;

  for (let i = 0; i < 232; i++) {
    const station = HSR_STATIONS[i % HSR_STATIONS.length];
    const room = rooms[i % rooms.length];
    const sys = systems[i % systems.length];
    const isApproved = i < 54;
    const isUnderSys = i >= 54 && i < 82;
    const isCjv = i >= 82 && i < 126;
    const isSmo = i >= 126 && i < 146;
    const isHnwl = i >= 146 && i < 215;
    ...
    rows.push({
      id: `tech-room-${counter}`,
      station: station,
      roomName: `${room} Level ${((i % 2) + 1)}`,
      system: sys,
      docNo: `HSR-SOA-DRW-TR-${station.substring(0, 3).toUpperCase().replace(/\s/g, '')}-${(counter).toString().padStart(4, '0')}`,
      rev: isApproved ? 'B' : 'A',
      statusHoneywell: statusHnwl,
      statusDate: '2026-03-05',
      wfNo: isApproved || isUnderSys ? `WF-${(25000 + counter).toString()}` : '-',
      statusWithSystra: statusSys,
      comment: isApproved ? 'Code 2 Approved' : isUnderSys ? 'Under Review by Systra Lead' : 'Technical room layout in revision cycle',
    });
    counter++;
  }
